...
def add_friend(username):
    ...
    # avoid duplicates/past accepted
    existing_pending = FriendRequest.query.filter(
        ((FriendRequest.from_id==current_user.id) & (FriendRequest.to_id==other.id)) |
        ((FriendRequest.from_id==other.id) & (FriendRequest.to_id==current_user.id))
    ).filter(FriendRequest.status=="pending").first()
    if existing_pending:
        flash("Request already pending.", "info")
    else:
        fr = FriendRequest(from_id=current_user.id, to_id=other.id, status="pending")
        db.session.add(fr)
        db.session.commit()
        flash("Friend request sent.", "success")
    return redirect(request.referrer or url_for("friends.list_and_find"))
...
def accept(req_id):
    ...
    if fr.to_id != current_user.id or fr.status != "pending":
        ...
        return redirect(url_for("friends.list_and_find"))
    # make friendship (both directions for convenience)
    me = current_user
    other = User.query.get(fr.from_id)
